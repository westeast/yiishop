			...
			<tr>
				...
				<td>
					<?php 
						...
						if($key>0&&($item->lft!=$item->parent_node->lft+1)){echo CHtml::link('<img src="/style/admin/images/up.gif">',array('/admin/type/moveUp/','id'=>$item->id));}
					?>
				<td>
					<?php
						$item->parent_node->rgt=isset($item->parent_node->rgt)?$item->parent_node->rgt:0;
						if($item->id!=$item->last_brother->id&&($item->rgt!=$item->parent_node->rgt-1)){echo CHtml::link('<img src="/style/admin/images/down.gif">',array('/admin/type/moveDown/','id'=>$item->id));}
					?>
				</td>
				...
				<td><?php echo CHtml::link('<img src="/style/admin/images/edit.gif">',array('/admin/type/edit/','id'=>$item->id)); ?> <?php echo CHtml::link('<img src="/style/admin/images/del.gif">',array('/admin/type/delete/','id'=>$item->id),array('class'=>'delete','id'=>$item->id)); ?></td>
			</tr>
		<?php }?>
		<tr>
